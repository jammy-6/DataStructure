...
class Solution {
  public:
    void build(TreeNode *&root, vector<int> &inorder, int il, int ir,
        ...
        // 切割中序数组
        build(root->left, inorder, il, idx - 1, postorder, pl,
              pl + (idx - il) - 1);
        build(root->right, inorder, idx + 1, ir, postorder, pl + (idx - il),
              pr - 1);
        return;
    }
    TreeNode *buildTree(vector<int> &inorder, vector<int> &postorder) {
        TreeNode *root;
        build(root, inorder, 0, inorder.size() - 1, postorder, 0,
              postorder.size() - 1);
        return root;
    }
    // TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder) {
    ...
    //     //切割中序数组
    //     vector<int> in_left  =
    //     vector<int>(inorder.begin(),inorder.begin()+idx); vector<int>
    //     in_right = vector<int>(inorder.begin()+idx+1,inorder.end());
    //     //切割后续数组
